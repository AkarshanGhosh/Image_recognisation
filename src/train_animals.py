...
# Simpler transformations for validation/test data
...
train_path = os.path.join(project_root, 'real_dataset', 'train', 'animals')
test_path = os.path.join(project_root, 'real_dataset', 'test', 'animals')
...
classes = trainset.classes  # Automatically inferred from folder names
...
def visualize_samples(dataset, classes, n=5):
    ...
    for i, c in enumerate(classes[:3]):  # Show first 3 classes
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(project_root, 'samples.png'))
    plt.close()

# Function to train the model with validation
def train_model(model, criterion, optimizer, scheduler, epochs=50, early_stop_patience=7):
    best_acc = 0.0
    best_epoch = 0
    patience_counter = 0
    ...
    for epoch in range(epochs):
        ...
        if accuracy > best_acc:
            ...
            best_epoch = epoch
            patience_counter = 0
            torch.save(model.state_dict(), os.path.join(models_dir, 'best_cnn_animals.pth'))
    ...
    # Final model save
    torch.save(model.state_dict(), os.path.join(models_dir, 'final_cnn_animals.pth'))
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(project_root, 'training_history.png'))
    plt.close()
...
if __name__ == "__main__":
    ...
    model.load_state_dict(torch.load(os.path.join(models_dir, 'best_cnn_animals.pth')))
